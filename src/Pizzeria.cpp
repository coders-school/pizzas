...
Pizzeria::Pizzeria(std::string const& name, Timer& timer) : name_(name), orders_(), timer_(timer) {}
...
double Pizzeria::calculatePrice(int orderId) {
    auto order = std::find_if(orders_.begin(), orders_.end(),
                              [orderId](const auto& order) { return std::get<0>(order) == orderId; });
    if (order != orders_.end()) {
        ...
        return std::accumulate(pizzas.begin(), pizzas.end(), 0.0,
                               [](double previousSum, Pizza* pizza) { return previousSum + pizza->getPrice(); });
    } else {
...
void Pizzeria::bakePizzas(int orderId) {
    ...
    if (order != orders_.end()) {
        ...
        for (const auto& pizza : pizzas) {
            ...
            timer_.sleepFor(pizza->getBakingTime());
        }
